// ==========================================================
// POR DO SOM — render JS (vitrine, gravadora, audiovisual)
// ==========================================================
(async function(){
  let CAT;
  try {
  ...
  if (vitrine) {
    ...
    vitrine.innerHTML = destaques.map(a => `
      <a href="/albuns/${a.slug}.html" title="${a.titulo} — ${a.artista}">
        <img src="${a.capa}" alt="Capa: ${a.titulo}" loading="lazy" onerror="this.src='${fallback()}'">
  ...
  if (filtrosEl && grid) {
    ...
    function renderGrid(g){
      ...
      grid.innerHTML = lista.map(a => `
        <a class="album-card" href="/albuns/${a.slug}.html">
          <div class="album-capa">
            <img src="${a.capa}" alt="Capa: ${a.titulo}" loading="lazy" onerror="this.src='${fallback()}'">
          </div>
          <div class="album-info">
            <div class="album-titulo">${a.titulo}</div>
            <div class="album-artista">${a.artista} · ${a.ano || ''}</div>
            <div class="album-tags">${a.generos.map(g => `<span class="album-tag">${nomeG(g)}</span>`).join('')}</div>
          </div>
        </a>`).join('') || '<p style="grid-column:1/-1;text-align:center;color:var(--text-muted)">Nenhum álbum neste gênero ainda.</p>';
    }
  ...
  if (socialGrid && CAT.contato) {
    ...
    const redes = [
      ['instagram', 'Instagram', CAT.contato.instagram_handle],
